...
document.addEventListener("keydown", (event) => {
  ...
  const changeTheme = () => {
    if (!isDarkTheme) {
      ...
      for (let item in allTaskItems) {
        allTaskItems[item].style.color = "#fff";
      }

      for (let button in allButtons) {
        allButtons[button].style.border = "1px solid #fff";
      }
      ...
      for (let item in allTaskItems) {
        allTaskItems[item].style.color = "initial";
      }

      for (let button in allButtons) {
        allButtons[button].style.border = "none";
      }
...
let newTask = () => {
  tasks.forEach((el) => {
    ...
    // Если темиа тёмная, то отрисовываем новые элементы с соответствующими стилями
